if (typeof (layer) != "undefined") {
    ...
    s.rel = "stylesheet";
    s.href = myuserhost + "/css/haslayer.css";
    a.appendChild(s);
} else {
    ...
    ;!function(e, t) {
        ...
        var i, n, a = e.layui && layui.define, o = {
            ...
            ready: function(e) {
                var t = "layer"
                    , i = ""
                    , n = "/user/css/layer.css?v=1" + r.v + i;
...
function logout_submit() {
    $.ajax({
        ...
        success: function () {
            location.href = myuserhost + '/login';
        }
    })
}
...
function clearUserMsg(id) {
    if (g_irefer) {
        ...
        $.ajax({
            ...
            type: "GET",
            async: false,
            //使用同步的方式,true为异步方式,这里需要设置成false,不然跳转页面乱码
            data: {
                id: id
            },
            dataType: 'jsonp',
...
$(".header_wrap .header button").css({
    "background": "#f19149"
});
$(".header_wrap .header input").css({
    "background": "#f2f2f2",
    "border-color": "#dbdbdb"
});
...
function checkuserlogin_back(data) {
    if (data.code == 1) {
        ...
        userinfo += '</div>';
        userinfo += '</div>';
        if (typeof (data.mysign) != "undefined" && data.mysign.today_date != "") {
            userinfo += '<div class="Sign_bt"><a href="javascript:void(0);"><span class="ico_sign_task ico_sign"></span>签到</a></div>';
            userinfo += '<div class="signwarp">';
            userinfo += '<div class="timebbox">';
            userinfo += '<div class="time_date">' + data.mysign.today_date + '</div>';
            userinfo += '<div class="time_cent">';
            userinfo += '<div class="day">' + data.mysign.today_day + '</div>';
            userinfo += '<div class="week">' + data.mysign.today_week + '</div>';
            userinfo += '</div>';
            userinfo += '<div class="time_text">';
            userinfo += '<p>' + data.mysign.nongli_month + '</p>';
            userinfo += '<p> ' + data.mysign.nongli_year + '年 【' + data.mysign.nongli_shengxiao + '年】</p>';
            userinfo += '</div>';
            userinfo += '</div>';
            userinfo += '<div class="text_">';
            userinfo += '<div class="bt_top">累计签到 <p><span>' + data.mysign.sign_total + '</span> 天</p></div>';
            userinfo += '<div class="tale">';
            userinfo += '<div class="every_day">目前连续签到' + data.mysign.contin_sign_total + '天</div>';
            if (data.mysign.contin_sign_total > 1) {
                userinfo += '<div class="exper">经验  +10</div>';
            } else {
                userinfo += '<div class="exper">经验  +5</div>';
            }
            userinfo += '</div>';
            userinfo += '<div class="weekly">';
            userinfo += '<div class="bt">周进度</div>';
            userinfo += '<div class="starbox"><div class="star_line" style="width: ' + parseInt(data.mysign.week_sign_total * 100 / 7) + '%;"></div></div>';
            if (data.mysign.has_sign == 1) {
                userinfo += '<div class="btn_sign on">已签到</div>';
            } else {
                userinfo += '<div class="btn_sign" onclick="user_signday()">签到</div>';
            }
            userinfo += '</div>';
            userinfo += '</div>';
            userinfo += '</div>';
        }
        if (typeof (data.mytask) != "undefined" && data.mytask.length > 0) {
            userinfo += '<div class="task_bt"><a href="javascript:void(0);"><span class="ico_sign_task ico_task"></span>每日任务</a></div>';
            userinfo += '<div class="taskwarp">';
            userinfo += '<div class="bttitle"><div class="bt">每日任务</div></div>';
            userinfo += '<div class="list_">';
            for (var itask = 0; itask < data.mytask.length; itask++) {
                var mytask = data.mytask[itask];
                userinfo += '<div class="lis">';
                userinfo += '<div class="lab1">';
                userinfo += '<div class="title_text">';
                userinfo += '<p>' + mytask.tips + '</p>';
                userinfo += '<div class="exper">' + mytask.rewards + ' 经验</div>';
                userinfo += '</div>';
                if (mytask.is_available == 1) {
                    userinfo += '<div class="btn_task on" onclick="user_gettask(this,' + mytask.type + ')">领取</div>';
                } else if (mytask.is_available == 2) {
                    userinfo += '<div class="btn_task">已领</div>';
                } else {
                    userinfo += '<div class="btn_task">领取</div>';
                }
                userinfo += '</div>';
                userinfo += '<div class="lab2">';
                userinfo += '<div class="starbox"><div class="star_line" style="width: ' + parseInt(mytask.my_total * 100 / mytask.need_total) + '%;"></div></div>';
                userinfo += '<div class="score">' + mytask.my_total + '/' + mytask.need_total + '</div>';
                userinfo += '</div>';
                userinfo += '</div>';
            }
            userinfo += '</div>';
